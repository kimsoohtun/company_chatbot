...
def extract_text_from_folder(folder_path, g_url):
    ...
    # 지정된 폴더(data) 내의 파일들을 자동으로 읽기 [cite: 2]
    if os.path.exists(folder_path):
        for filename in os.listdir(folder_path):
            file_path = os.path.join(folder_path, filename)
            content = ""
            try:
                if filename.endswith('.pdf'): [cite: 3]
                    with pdfplumber.open(file_path) as pdf:
                        content = "\n".join([p.extract_text() for p in pdf.pages if p.extract_text()])
                elif filename.endswith('.docx'):
                    content = "\n".join([p.text for p in Document(file_path).paragraphs])
                elif filename.endswith('.xlsx'): [cite: 4]
                    content = pd.read_excel(file_path).to_string()
                
                if content:
                    text_data += f"\n\n[출처: {filename}]\n{content}"
                    sources.append(filename) [cite: 5]
    ...
    return text_data, sources [cite: 7]
...
# --- 4. 질문 처리 ---
if prompt := st.chat_input("궁금한 규정을 입력하세요."):
    if not api_key:
        st.error("관리자 설정(API Key)이 되지 않았습니다. Secrets 설정을 확인하세요.")
    else:
        st.session_state.messages.append({"role": "user", "content": prompt})
        with st.chat_message("user"):
            st.markdown(prompt)

        with st.chat_message("assistant"): [cite: 9]
            try:
                model = genai.GenerativeModel('gemini-2.5-flash')
                full_query = f"""너는 사내 규정 전문가야. 아래 지식 베이스를 바탕으로 답변해줘.
                답변 끝에 '참고 문서: [문서명]'을 꼭 적어줘. 
                모르는 내용은 반드시 '인사팀에 문의하세요'라고 답변해. [cite: 10]
                ...
                response = model.generate_content(full_query) [cite: 11]
